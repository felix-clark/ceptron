...
namespace ceptron {
  ...
  void trainSlfnStatic(Net& net, IMinStep& ms,
    ...
    func_t f = std::bind(costFunc<Net>, std::placeholders::_1, x, y, l2reg);
    fg_t fg = std::bind(costFuncAndGrad<Net>, std::placeholders::_1, x, y, l2reg);
    grad_t g = [&](const ArrayX& in){return fg(in).g;}; // strips gradient from combined result
