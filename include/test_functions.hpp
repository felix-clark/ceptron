...
scalar rosenbrock(const Array<N>& pars, scalar scale = 100.0) {
  ...
  auto result = scale * (last - first.square()).square().sum() +
                (1 - first).square().sum();
...
Array<N> grad_rosenbrock(const Array<N>& pars, scalar scale = 100.0) {
  ...
  shift_forward.template segment<N - 1>(1) =
      pars.template segment<N - 1>(0);  // shift_forward[k] == pars[k-1]
  shift_back.template segment<N - 1>(0) =
      pars.template segment<N - 1>(1);  // shift_back[k] == pars[k+1]

  Array<N> grad = scale * 2.0 * (pars - shift_forward.square() -
                                 2 * pars * (shift_back - pars.square())) -
                  2 * (1 - pars);
  // first and last elements are special because they are not coupled
  grad[0] =
      -4 * scale * pars[0] * (pars[1] - pars[0] * pars[0]) - 2 * (1 - pars[0]);
