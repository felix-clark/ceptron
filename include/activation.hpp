...
// but it's a pedagogical choice so we'll keep it as an option. (also used in
// log reg, tho output nodes are currently specialized)
// Tanh is better (since it's centered around 0) but rectified linear units
// (ReLU) and variations are popular now.
// Softplus is a smooth version of an ReLU but is not as trivial to get the
// derivative of.
enum class InternalActivator {
  Identity,
  Logit,
  Tanh,
  ReLU,
...
/// a number of activation functions can be generalized with hyperparameters,
/// even on a per-channel level.
// implementing this will require some interface changes, but it's not yet a
// priority in terms of making this library useful.
// see https://arxiv.org/pdf/1710.05941.pdf for an exploration of various other
// activation functions

// see https://eigen.tuxfamily.org/dox/TopicFunctionTakingEigenTypes.html
// though we did have trouble working with Eigen::ArrayBase<Derived>, even at
// times getting incorrect values despite compilation (!)

// a version w/ runtime lookup of the activation choice
// there's no reason to not use the dynamic BatchArrayX, except perhaps for when
// a single BatchArray is passed ?
// it may end up being feasible to do the calculations in-place in some cases,
// but commiting to that model may not be worth the loss of potential
// flexibility
ceptron::BatchArrayX activ(InternalActivator,
                           const Eigen::Ref<const ceptron::BatchArrayX>&);
ceptron::BatchArrayX activToD(InternalActivator,
                              const Eigen::Ref<const ceptron::BatchArrayX>&);

template <InternalActivator Act>
class ActivFunc {
 public:
  // there should possibly be some additional voodoo to ensure that ArrT is an
  // Eigen::ArrayBase
  // note that that seemed to bite us when we tried
  // these need to be static (and therefore not const) in order to call them
  // without an instance
  template <typename ArrT>
  static inline ArrT activ(const ArrT& in);
  template <typename ArrT>
  static inline ArrT activToD(const ArrT& act);  // sig -> sig', not x -> sig'
  // another function like dactive_from_in would compute the derivative directly
  // from the input,
  //  which might be necessary for the less nice activation choices
  // we could consider returning both the activated layer and it's derivative
  // simultaneously
};
...
/// something like ArrT -> Eigen::ArrayBase<ArrT> ? but that did lead to some
/// very strange results at one point so maybe not
ArrT ActivFunc<InternalActivator::Logit>::activ(const ArrT& in) {
  // // something like the following assertion would be good to restrict to
  // Array types, but I'm not getting it to work atm
...
ArrT ActivFunc<InternalActivator::ReLU>::activ(const ArrT& in) {
  // the methods evaluating the size of in shouldn't be called when using
  // static-sized inputs -- they're a redundant part of the interface for
  // convenience
...
ArrT ActivFunc<InternalActivator::ReLU>::activToD(const ArrT& act) {
  // maybe return 0.5 at zero exactly to give nets that are zero-initialized
  // some gradient
...
// this one could likely benefit from an approach where we compute the layer and
// its derivative simultaneously.
...
// the slope at x < 0 should really be an adjustable hyperparameter, and can
// even be tweaked on a per-channel basis.
...
ArrT ActivFunc<InternalActivator::Softsign>::activToD(const ArrT& act) {
  // this one is easier to compute in terms of input rather than the activation
  // itself
