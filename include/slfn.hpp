...
// could a base case using the CRTP help avoid some of the awkwardness in the
// definitions below?
template <typename Derived>
class SlfnBase {
 public:
  // static constexpr size_t size = Derived::size; // this pattern does actually
  // work. is it useful? // it actually fails in clang 3. we aren't using it now
  // anyway so we'll let it stay commented.
 private:
  SlfnBase() = default;  // making the constructor private and the derived class
                         // a friend ensures
  friend Derived;        // that a derived class can only give its own type as a
                         // parameter
};

// we will really want to break this apart (into layers?) and generalize it but
// first let's get a simple working example.
...
class SlfnStatic : public SlfnBase<SlfnStatic<N, M, P, Reg, Act>> {
 public:
  ...
  static constexpr size_t hiddens = P;  // this member won't generalize well, so
                                        // we should replace typedefs for 1st,
                                        // 2nd layer matrix types, for instance
 ...
 public:
  ...
  Array<N> inputLayerActivation(const Vec<N>& in) const;  // returns activation
                                                          ...
                                                          // these will no
                                                          // longer be cached
  Array<P> hiddenLayerActivation(const Vec<N>& in)
      const;  // they don't actually need to be member functions
  // using "auto" may not be best when returning a map view -- TODO: look into
  // this more?
  auto getFirstSynapses(const ArrayX& net) const {
    return Map<const Mat<P, N + 1>>(net.data());
  };
  auto getSecondSynapses(const ArrayX& net) const {
    return Map<const Mat<M, P + 1>>(
        net.template segment<size_w2_>(size_w1_).data());
  };
...
Array<Net::size> randomWeights() {
  ...
  Array<Net::size> net = Array<Net::size>::Zero();  // set all to zero - biases
                                                    // should be initialized to
                                                    // zero
  // Eigen uses rand, so the random numbers can be seeded by a call to
  // std::srand().
  // the weight variances should be scaled down by a factor of the square root
  // of the # of their inputs,
  //  so that the variance of their output is not a function of the number of
  //  inputs.
  // Map< Mat<P, N> >((net.template segment<P*N>(P)).data()) = Mat<P,
  // N>::Random()/sqrt(N);
  // Map< Mat<M, P> >((net.template segment<M*P>(P*(N+1)+M)).data()) = Mat<M,
  // P>::Random()/sqrt(P);
  // Glorot and Bengio suggest the following initialization instead:
  Map<Mat<P, N>>((net.template segment<P * N>(P)).data()) =
      Mat<P, N>::Random() * sqrt(6.0 / (N + P));
  Map<Mat<M, P>>((net.template segment<M * P>(P * (N + 1) + M)).data()) =
      Mat<M, P>::Random() * sqrt(6.0 / (P + M));
  return net;
}

// we should be able to build up multi-layer networks by having each layer work
// as its own and having their forward and backward propagations interact with
// each other

// possibly we can move this back to being a member variable, but it's not clear
// to me rn what would be gained/lost.
...
func_grad_res costFuncAndGrad(const Net& net, const ArrayX& netvals,
  ...
  if (Net::RegType == RegressionType::Categorical) {
    ...
    if ((y.colwise().sum().array() > 1.0).any()) {
      ...
      LOG_DEBUG(
          "classification data breaks unitarity. this net assumes mutually "
  ...
  // at some point we might wish to experiment with column-major (default) vs.
  // row-major data storage order
  BatchVec<P> a1 =
      w1.template leftCols<1>() * BatchVec<1>::Ones(1, batchSize)  // bias terms
      + w1.template rightCols<N>() * x0;  // weights term
  // apply activation function element-wise; need to specify the template type
  BatchVec<P> x1 =
      ActivFunc<Net::ActType>::template activ<BatchArray<P>>(a1.array())
          .matrix();
  ...
  BatchVec<M> x2 =
      Regressor<Net::RegType>::template outputGate<BatchArray<M>>(a2.array())
          .matrix();
  ...
  BatchVec<P> e1 =
      ActivFunc<Net::ActType>::template activToD<BatchArray<P>>(x1.array())
          .matrix();
  BatchVec<P> d1 =
  ...
  Mat<P, N> gw1 = d1 * x0.transpose() +
                  2.0 * net.getL2Reg() *
                      w1.template rightCols<N>();  // add regularization term
  // Vec<M> gb2 = d2.rowwise().sum();
  Mat<M, P> gw2 =
      d2 * x1.transpose() + 2.0 * net.getL2Reg() * w2.template rightCols<P>();
...
// we need to be careful w/ this function because it's similar to the version w/
// gradient, but stops sooner.
// a compositional style of this function inside the one that includes the
// gradient doesn't work trivially since the backprop needs some intermediate
// results from this calculation
template <typename Net>
scalar costFunc(const Net& net, const ArrayX& netvals,
                const BatchVec<Net::inputs>& x0,
                const BatchVec<Net::outputs>& y) {
  constexpr size_t N = Net::inputs;
  constexpr size_t M = Net::outputs;
  ...
  // we won't do as many checks in this version -- the whole point is to be
  // fast.
  const auto batchSize = x0.cols();
  // propagate forwards
  Mat<P, N + 1> w1 = net.getFirstSynapses(netvals);
...
template <typename Net>
Vec<Net::outputs> prediction(const Net& net, const ArrayX& netvals,
                             const Vec<Net::inputs>& x0) {
  constexpr size_t N = Net::inputs;
  constexpr size_t M = Net::outputs;
  constexpr size_t P = Net::hiddens;
  ...
  Vec<P> a1 = w1.template leftCols<1>()           // bias terms
              + w1.template rightCols<N>() * x0;  // weights term
  Vec<P> x1 = ActivFunc<Net::ActType>::template activ<Array<P>>(a1.array())
                  .matrix();  // net output of layer 1
  Vec<M> a2 = w2.template leftCols<1>() + w2.template rightCols<P>() * x1;
  Vec<M> x2 = Regressor<Net::RegType>::template outputGate<Array<M>>(a2.array())
                  .matrix();
  return x2;
}
