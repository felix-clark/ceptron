...
// not sure  yet that we actually need a separate class for this as opposed to
// using first-layer only,
//  but trimming down the interface is probably worth it.
class FfnDyn {
 public:
  template <typename... Ts>
  FfnDyn(RegressionType reg, InternalActivator act, Ts... layer_sizes);
  ~FfnDyn() = default;
  FfnDyn(const FfnDyn&) = delete;  // we do some non-trivial memory operations
                                   // w/ shared_ptrs so don't allow copying (not
                                   // sure if it works)
  // FfnDyn(FfnDyn&&) = delete; // move ops are not defined when we implement or
  // delete the copy-constructor
  // might need to disable operator= too?
  int num_weights() const {
    return size_;
  }  // number of weights in net (size of gradient)
  int numInputs() const;
  int numOutputs() const;

  // set an l2 regularization parameter, which adds a sum-of-squares of weight
  // terms (but not biases) to the cost function.
  void setL2Reg(scalar lambda);  // technically could be declared const, since
                                 // only the layers will care

  // convenience function to return a parameter array with weights (but not
  // biases) randomized,
  //  as a suggested initialization. numbers are scaled to have a variance that
  //  does not depend on number of inputs.
  //  use std::srand before calling to set the RNG seed.
  ArrayX randomWeights() const;

  // these methods are the workhorses of the NN, and are implemented recursively
  // on the layers
  scalar costFunc(const ArrayX& netvals, const BatchVecX& xin,
 ...
 private:
  ...
  // a "Layer" here refers to the structure that includes the weight matrix
  // taking "inputs_" values and mapping to "outputs_" values, with an
  ...
  class Layer {
   public:
    ...
    template <typename... Ts>
    Layer(InternalActivator act, RegressionType reg, size_t ins, size_t n1,
          size_t n2, Ts... sizes);
    ...
    scalar costFuncRecurse(const Eigen::Ref<const ArrayX>& net,
                           const BatchVecX& xin, const BatchVecX& yin) const;
    // returns the matrix needed for backprop
    // takes a non-const reference to fill the gradient with (hence "get" in the
    // function name)
    MatX getCostFuncGradRecurse(const Eigen::Ref<const ArrayX>& net,
                                const BatchVecX& xin, const BatchVecX& yin,
                                Eigen::Ref<ArrayX> gradnet) const;
   ...
   private:
    ...
    size_t outputs_;
    size_t num_weights_;
    // the output layer won't need to access these. in fact they will be
    // somewhat wasted memory.
    // if it was significant we could split this into InputLayer, but that's
...
template <typename... Ts>
FfnDyn::FfnDyn(RegressionType reg, InternalActivator act, Ts... layer_sizes) {
