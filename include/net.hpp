...
// for non-exclusive classification, independent NNs could be used.
// another template argument could specify whether it's a classifier or least-squares, which should actually change surprisingly little code.
// another template argument should indicate the internal activation functions:
//    e.g. sigmoid (bad IRL), tanh, ReLU, softplus, etc. (softsign, arctan,...)
...
public:
  ...
  double getOutput() const; // returns cache
  ...
  void propagateData(const Vec<N>& x0, double y);
  ...
  const auto& getNetValue() const {return net_.eval();}
...
template <size_t N, size_t M>
void SingleHiddenLayer<N,M>::resetCache() {
  input_layer_cache_ = Vec<N+1>::Zero();
  input_layer_cache_(0) = 1.; // bias term
  hidden_layer_cache_ = Vec<M+1>::Zero();
  hidden_layer_cache_(0) = 1.; // bias term
  output_layer_cache_ = 0.;
  output_value_cache_ = 0.;
...
template <size_t N, size_t M>
double SingleHiddenLayer<N,M>::getOutput() const {
  return output_layer_cache_;
}
...
template <size_t N, size_t M>
void SingleHiddenLayer<N,M>::propagateData(const Vec<N>& x0, double y) {
  ...
  Mat<1, M+1> w2 = getSecondSynapses(); // const references shouldn't be necessary for the expression templates to work
  ...
  Vec<1> a2 = w2 * hidden_layer_cache_;
  output_layer_cache_ = sigmoid(a2(0,0))/*eval*/; // this is sort-of implicit logistic regression; 1D version of softmax
  // return output_layer_cache_; // this version is outputting a scalar; just access it directly
  ...
  Vec<1> delta_out;// = output_layer_cache_ - y;
  delta_out << (output_layer_cache_ - y);
  ...
  auto grad_w2 = Map< Mat<1, M+1> >(net_grad_cache_.template segment<size_w2_>(size_w1_).data());
...
template <size_t N, size_t M>
double SingleHiddenLayer<N,M>::getCostFuncVal() const {
  /// - (output_value_cache_.array()*log(output_layer_cache_.array())).sum(); // multi-dimensional case
  ...
  return  - output_value_cache_*log(output_layer_cache_)
    - (1-output_value_cache_)*log(1-output_layer_cache_); // 1-dimensional case
}

// template <size_t N, size_t M>
// // would be nice to be able to write Array<size_> in the return signature
// Array<M*(N+1)+(M+1)> SingleHiddenLayer<N,M>::getCostFuncGrad() const {
//   return net_grad_cache_;
// }
