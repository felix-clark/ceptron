...
void ceptron::trainFfnDyn(const FfnDyn& net, ArrayX& par, IMinStep& ms,
  ...
  // to get pointer to member function, std::bind takes additional argument that
  // is the address of the member to call it from.
  // a lambda would likely work just as well.
  func_t f = std::bind(&FfnDyn::costFunc, &net, std::placeholders::_1, x, y);
  grad_t g =
      std::bind(&FfnDyn::costFuncGrad, &net, std::placeholders::_1, x, y);
