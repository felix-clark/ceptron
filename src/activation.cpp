...
ceptron::BatchArrayX ceptron::activ(InternalActivator act,
                                    const Eigen::Ref<const BatchArrayX>& in) {
  switch (act) {
    case InternalActivator::Identity:
      return ActivFunc<InternalActivator::Identity>::activ(in);
...
ceptron::BatchArrayX ceptron::activToD(InternalActivator act,
                                       const Eigen::Ref<const BatchArrayX>& x) {
  switch (act) {
    case InternalActivator::Identity:
      return ActivFunc<InternalActivator::Identity>::activToD(x);
    case InternalActivator::Logit:
  ...
  throw std::runtime_error(
      "unimplemented runtime activation derivative function");
}
